...
let rec do_transaction task_id hf bp =
  try
    Buffer_pool.with_tid bp (fun tid ->
      ...
      Heap_file.insert_tuple hf (U.make_counter_tuple (n + 1)) tid;
      Log.log_tid tid "task %d ended" task_id)
  with
  | Error.DBError Error.Deadlock_victim ->
    Log.log "deadlock detected on task %d, retrying..." task_id;
    Unix.sleepf 0.01;
