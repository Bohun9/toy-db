...
and build_plan_select
  ...
  =
  let table_expr, table_env = check_table_expr reg table_expr in
            ...
            (List.map
               (fun select_item ->
                  ...
                  | C.Syntax.SelectField { field } ->
                    Table_env.resolve_field table_env field
                  | C.Syntax.SelectAggregate _ -> raise Error.aggregate_without_grouping)
               select_list)
      in
      let select_list_fields =
        match select_list with
        | Star -> List.map Table_field.to_field (Table_env.fields table_env)
        | SelectFields fields -> List.map Table_field.to_field fields
      in
      NoGrouping { select_list }, select_list_fields
    | Some { group_by_fields } ->
      let group_by_fields =
        List.map (Table_env.resolve_field table_env) group_by_fields
      in
      let group_by_env = Group_by_env.of_fields group_by_fields in
      let select_list =
        match select_list with
        ...
        | C.Syntax.SelectList select_list ->
          List.map
            (fun select_item ->
               match select_item with
               | C.Syntax.SelectField { field } ->
                 let field, group_by_index =
                   Group_by_env.resolve_field group_by_env field
                 in
      ...
      let select_list_fields =
        List.map
          (fun select_item ->
             match select_item with
             | SelectField { field; _ } -> Table_field.to_field field
  ...
  in
  let select_list_env = Select_list_env.of_fields select_list_fields in
  let order_specifiers =
    Option.map
      (fun order_list ->
         List.map
           (fun ({ field = field_name; order } : C.Syntax.order_specifier) ->
              { field = Select_list_env.resolve_field select_list_env field_name
